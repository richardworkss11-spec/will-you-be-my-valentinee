    ...
    >
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          {valentine.location && (
            <p className="text-xs text-rose-600/60 mt-0.5 flex items-center gap-0.5">
              📍 {valentine.location}
            </p>
          )}
        </div>
      ...
      {/* Submission date */}
      <div className="mt-3 pt-3 border-t border-white/30 text-xs text-rose-600/50 font-medium">
        {new Date(valentine.created_at).toLocaleDateString(undefined, {
          month: "short",
          day: "numeric",
          year: "numeric",
        })}
      ...
      {valentine.reaction && (
        <div className="absolute bottom-3 right-3 bg-white/80 backdrop-blur-sm rounded-full px-2.5 py-1 text-lg shadow-sm border border-white/50">
          {valentine.reaction}
        </div>
      )}
